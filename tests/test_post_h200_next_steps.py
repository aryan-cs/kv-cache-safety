...
def test_post_h200_next_steps_routes_wide_ci_to_registered_extension() -> None:
    report = post_h200_next_steps(
        {
            "publication_ready": False,
            "blockers": ["primary_results_complete"],
            "gates": {
                "primary_results_complete": False,
            ...
            "primary_results": {
                ...
                "disqualifiers": [],
                "readiness_failures": [
                    "kv_int4_sim:policy_level_ssei_ci_width=0.170; target<=0.080"
                ],
            },
            "causal_results": {
                "missing": [],
    ...
    assert report["next_step"] == "resolve_ci_width"
    assert report["steps"][0]["state"] == "complete"
...
def test_post_h200_next_steps_commands_follow_selected_paths() -> None:
    report = post_h200_next_steps(
        {
            ...
            "primary_generated": {"path": "docs/generated/merged_primary_assets"},
            "causal_generated": {"path": "docs/generated/selected_causal_assets"},
            "primary_human_audit": {
                "path": "docs/audit/h200_qwen_full_sweep_plus_ci_extension_summary"
            },
            "causal_human_audit": {"path": "docs/audit/h200_causal_patch_qwen7b_summary"},
            "claim_assessment": {"path": "docs/generated/claim_assessment/claim_assessment.json"},
            "arxiv_bundle": {
                "source_dir": "docs/build/promoted_arxiv_source",
                "archive": "docs/build/promoted_arxiv_source.tar.gz",
            },
        }
    )
    rendered = render_markdown(report)
    ...
    assert "docs/generated/merged_primary_assets" in rendered
    assert "docs/generated/selected_causal_assets" in rendered
    assert "docs/generated/h200_qwen_full_sweep_plus_ci_extension" not in rendered
    assert "PRIMARY_GENERATED_DIR=docs/generated/merged_primary_assets" in audit_command
    assert "CAUSAL_GENERATED_DIR=docs/generated/selected_causal_assets" in audit_command
    assert "PRIMARY_GENERATED_DIR=docs/generated/merged_primary_assets" in build_command
    assert "CAUSAL_GENERATED_DIR=docs/generated/selected_causal_assets" in build_command
    assert "ARXIV_SOURCE_DIR=docs/build/promoted_arxiv_source" in build_command
    assert "ARXIV_ARCHIVE=docs/build/promoted_arxiv_source.tar.gz" in build_command
    assert (
