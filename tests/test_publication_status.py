...
def test_publication_status_defaults_follow_docs_tree() -> None:
    script = Path("scripts/report_publication_status.py").read_text(encoding="utf-8")
    files = _required_arxiv_bundle_files()

    assert "paper/generated" not in script
    assert "paper/audit" not in script
    assert "paper/build" not in script
    assert 'default=Path("docs/generated/h200_qwen_full_sweep")' in script
    assert 'default=Path("docs/audit/h200_qwen_full_sweep_summary")' in script
    assert 'default=Path("docs/build/arxiv_source")' in script
    assert 'FINAL_PDF_NAME = "kv-cache-safety.pdf"' in script
