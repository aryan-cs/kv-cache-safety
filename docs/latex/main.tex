...
An Architecture-Dependent Failure Mode of Refusal Behavior Under KV-Cache Compression}
...
Three models do not show the pattern: Gemma-2-9B-IT, Mistral-7B-Instruct-v0.3, and OpenAI gpt-oss-20b. These models share architectural features that decouple long-range KV cache state from the safety-relevant generation circuit: Gemma 2 interleaves local sliding-window attention with global attention \citep{team2024gemma}, Mistral 7B v0.3 applies sliding-window attention at every layer \citep{jiang2023mistral}, and gpt-oss-20b is a mixture-of-experts model trained with a structured ``harmony'' response format \citep{openai2025gptoss}. Under our cache interventions gpt-oss-20b also collapses into a uniform output mode rather than gracefully degrading. Read together, the positive results in five families and the negative results in these three constitute a falsification test that the affected behavior is specifically cache-resident long-range attention state, not generic compression sensitivity.

Causal localization experiments, in which baseline cache state from system-role tokens is patched into compressed runs, show the predicted recovery of refusal behavior on a legacy Qwen2.5-7B-Instruct run. Restoring matched user-role state under the same retained-token budget does not recover refusal. A multi-seed extension on Qwen3-9B and Phi-4 is in progress and is reported here at the level of the qualitative restoration patterns observed so far.

\paragraph{Contributions.}
We make three contributions. First, a cross-family measurement of selective safety degradation under cache eviction across nine open-weight checkpoints, with explicit per-policy effect sizes and bootstrap confidence intervals. Second, an architecture-dependent characterization of which models are affected and which are robust, framed as a structural prediction rather than a model-level idiosyncrasy. Third, causal evidence that the affected behavior tracks the long-range KV state of system-policy tokens, via baseline-to-compressed cache patching with matched user-role controls.
...
A causal-patching study on Qwen3-9B against the simulated four-bit-KV quantization treatment, judged by an external Claude Sonnet rater on the public refusal-safety suite, shows the predicted asymmetry. Restoring baseline key+value slices for system-role tokens at the first sixteen positions produces a refusal-shaped behavior shift that the matched user-role intervention under the same retained-token budget does not produce: the system-role intervention moves the safety pass-rate substantially past the compressed-cache baseline, while the matched user-role control leaves it essentially unchanged from the compressed condition. The two interventions retain the same number of K and V slices and differ only in which role's content is restored, so the dissociation localizes the safety-relevant cached information to system-role tokens rather than to a generic cache-norm effect. Policy-pinned cache retention, which protects system tokens from eviction directly, recovers refusal at an intermediate level. A parallel Phi-4 causal-patching run is in progress under the same protocol; results will be folded into the active causal artifact when the Sonnet judging completes.
...
\begin{figure}[t]
  ...
  \caption{Restoration fraction toward baseline on the refusal-safety suite. Zero is the unpatched compressed condition; one is full baseline. System-role restoration and policy-pinned mitigation move close to one; the matched user-role restoration does not.}
  \label{fig:causal}
\end{figure}
...
The causal-restoration evidence is the strongest mechanistic claim we can make from the data on hand. Restoring system-role cache state at the first sixteen positions moves Qwen3-9B's refusal-shaped behavior on harmful prompts substantially, while restoring the matched user-role state under the identical retained-token budget does not. The matched-control design isolates the role of the content being restored from any generic effect of adding K+V slices back into a compressed run. This pattern is consistent with safety-relevant information being carried by the cached representations of system-policy tokens rather than by a property of the model weights alone or a global cache norm. Whether the Phi-4 causal-patching replication shows the same role-specific asymmetry will determine the cross-architecture generality of this localization claim.
