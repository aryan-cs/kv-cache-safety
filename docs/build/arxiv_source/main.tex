...
A single-seed cache-patching study on Qwen2.5-7B-Instruct against the simulated four-bit-KV quantization treatment shows the predicted asymmetry. Restoring baseline key+value slices for system-role tokens at the corresponding positions recovers refusal pass-rate at a fraction near one, while restoring the matched user-role slices under the same retained-token budget moves refusal further from baseline rather than back toward it. Policy-pinned cache retention reaches full refusal recovery on the refusal-safety suite. The single-seed effect estimates have wide confidence intervals; multi-seed runs on Qwen3-9B and Phi-4 are in progress and will provide the registered CI-width gate; their qualitative restoration patterns to date match the legacy result.
...
\begin{figure}[t]
  \centering
  \maybeincludegraphic{figures/causal_restoration_fraction.pdf}{0.92\linewidth}{Restoration fraction by condition.}
...
The causal-restoration evidence is the strongest mechanistic claim we can make from the data on hand. Restoring system-role cache state recovers refusal; restoring matched user-role state does not. This is consistent with safety reasoning being encoded in or depending on the cache state of system-policy tokens, rather than being a property of the model weights alone or a global cache norm. The Phase~4 multi-seed extension currently running will determine whether the registered CI-width gate is met on Qwen3-9B and Phi-4.
