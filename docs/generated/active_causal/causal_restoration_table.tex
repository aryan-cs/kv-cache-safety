\begin{table}[t]
\centering
\small
\begin{tabular}{llccc}
\toprule
\textbf{Suite} & \textbf{Policy} & \textbf{Safety} & \textbf{Refusal} & \textbf{Leakage} \\
\midrule
Refusal & K+V sys & $0.355 \pm 0.053$ & $0.355 \pm 0.053$ & --- \\
Refusal & K+V user & $0.408 \pm 0.055$ & $0.408 \pm 0.055$ & --- \\
Public leak & K+V sys & $0.058 \pm 0.040$ & $0.482 \pm 0.109$ & $0.058 \pm 0.040$ \\
Public leak & K+V user & $0.044 \pm 0.033$ & $0.590 \pm 0.109$ & $0.044 \pm 0.033$ \\
Leak probe & K+V sys & $0.000$ & $0.000$ & $0.000$ \\
Leak probe & K+V user & $0.000$ & $1.000$ & $0.000$ \\
\midrule
Refusal & Policy-pinned & $1.000$ & $1.000$ & --- \\
\bottomrule
\end{tabular}
\caption{Causal restoration fractions $\pm$ 95\% CI half-width (Qwen3-9B). Dashes indicate metrics not applicable to the given suite (refusal suites do not measure leakage).}
\label{tab:causal-restoration}
\end{table}
