...
The primary model family is Qwen2.5 Instruct at 7B, 14B, and 32B parameters. The core behavioral evaluation uses a mid-scale instruct model, with smaller models used for causal diagnostics and larger models used as a scale check. A general model-family claim requires at least one non-Qwen open instruct model; otherwise, claims must be limited to the tested Qwen-family setting.

All experiments use open or free tooling only. Model loading uses bfloat16 weights, \texttt{safetensors}, and explicit cache-state access so that the evaluated interventions are visible in the generation loop rather than hidden inside opaque serving infrastructure.
...
The main figures are designed to make the proposed phenomenon visually legible, rather than only reporting aggregate scatterplots or bar charts. We use four figure families.

\paragraph{Cache-state fingerprints.}
For each policy-budget condition, the analysis renders a layer-by-token retention map with token roles on the horizontal axis and layers on the vertical axis. System, developer, user, template, and generated spans are shown as separate bands. A safety-specific erasure pattern should appear as a visible break or thinning in policy-token access, not merely as a lower scalar score.

\paragraph{Safety-capability phase portraits.}
Instead of a static scatterplot alone, each policy traces a trajectory through safety loss and capability loss as the cache budget tightens. The shape of the trajectory matters: a selective safety-erasure pattern should bend upward before it moves rightward, indicating safety degradation before broad capability collapse.

\paragraph{Restoration flow diagrams.}
Causal patching is shown as a flow from baseline to compressed decoding to system-patched, matched-user-patched, and policy-pinned conditions. The diagnostic pattern is a restoration stream returning toward the baseline only when policy-relevant cache slices are restored or protected.
...
\todoresult{Replace this box with figures and tables generated from completed analysis artifacts before making empirical claims.}
