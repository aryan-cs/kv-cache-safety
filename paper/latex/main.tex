...
The primary uncertainty report uses prompt-clustered confidence intervals. Main tables include paired safety degradation intervals by prompt and seed. The strongest claims require narrow intervals, complete prompt-policy coverage, and agreement between automated metrics and the declared audit source.
...
\begin{figure}[t]
  ...
  \caption{Policy-level uncertainty braid. Each row links capability degradation, safety degradation, and $\ssei$ intervals for one cache policy, exposing whether selective effects are both large and statistically tight.}
